...
class SpecializedMultiHeadAttention(nn.Module):
    ...
    def forward(self, x, input_ids, tokenizer, special_vocabs):
        ...
        # Store the scaled dot product scores for standard heads
        ...
        # Scaled dot product scores before masking
        ...
        # scores_spec is the masked scaled dot product score
        ...
        # Return final output, specialized attention weights, and the two sets of attention scores
        return final_output, attn_weights_spec, scores_std, scores_spec
...
if __name__ == "__main__":
    ...
    # The current setup only uses N_SPECIAL_HEADS. We will slice the vocabs list
    # to match the number of specialized heads for this example.
    ...
    # --- Run the Model ---
    # Capture the two new score tensors: standard_scores and specialized_scores
    output, special_attn_weights, standard_scores, specialized_scores = special_attention_block(
        x=input_embeddings,
        input_ids=input_ids,
        tokenizer=tokenizer,
    ...
    print(f"Special Attention Weights Shape (B, H_spec, S, S): {special_attn_weights.shape}")
